...
public class InventoryCommand implements CommandExecutor {
    ...
    AxSync reference;
    public InventoryCommand(AxSync reference){
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String @NotNull [] args) {
        ...
        if(!p.hasPermission("axsync.inventory")){
            p.sendMessage(ChatColor.RED + "You are missing the permission! [axsync.inventory]");
            return true;
        }
        if(args.length != 1){
