...
public class DatabaseManager {
    ...
    public boolean initDatabaseTable1(){
        try(Connection conn = getConnection()){
            try(PreparedStatement ps = conn.prepareStatement("CREATE TABLE IF NOT EXISTS playerdata (uuid VARCHAR(255), health DOUBLE, foodlevel INT, gamemode ENUM('SURVIVAL', 'CREATIVE', 'ADVENTURE', 'SPECTATOR'), saturation FLOAT, level INT, progress FLOAT, airlevel INT)")){
                ps.executeUpdate();
                return true;
            }
